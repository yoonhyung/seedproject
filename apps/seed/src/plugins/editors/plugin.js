...
define([(time = timedLogger.getLoggerTime(), 'text!./ext-to-mime.json'),
    'lodash',
    'util/path',
    'util/arrays/BubblingArray',
    'src/plugin-manager-0.1',
    'plugins/workbench/plugin',
    'widgets/views/view',
    'widgets/views/viewmanager',
    'widgets/views/viewFocusController',
    'dojo/topic',
    // 'other-lib/async',
    'util/logger/logger-client'
], function(extToMime, _, pathUtil, BubblingArray, pm, workbench,
    View, vm, ViewFocusController, topic, /*async,*/ Logger) {
    'use strict';
    ...
    function getFileManager() { // TODO: remove publish().
        ...
        FileManager.openFile = function(file) {
            ...
            editors.onFileOpened(file);
            var content = 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Velit, omnis atque tempore aliquid ab laboriosam sapiente, vitae natus excepturi dolor eveniet earum veritatis temporibus. Repellat voluptatibus quam, tenetur rem aut.';
            topic.publish('file.opened', file, content);
            file.savedValue = content;
        };
        ...
        FileManager.saveFile = function(file, option) {
            ...
            if (codeEditor.trimTrailingWhitespaces ||
                ...
                if (codeEditor.retabIndentations) {
                    ...
                    var unit = codeEditor.options.indentUnit,
                        re = /^(( )*)\t/m,
                        m;
                    while ((m = v.match(re))) {
