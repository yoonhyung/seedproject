...
define([
    'require',
    'plugins/editors/viable-menu-items',
    'lodash',
    ...
    codemirror.commands.blockcomment = function(cm) {
        if (cm.__instance) {
            ...
            var bcStart = mode.blockCommentStart,
                bcEnd = mode.blockCommentEnd;
            if (commenting) {
                doc.replaceRange('\n' + bcEnd, {
                    line: to.line,
                    ch: lastLine.length
                ...
                if (comments.length === 1) {
                    ...
                    doc.replaceRange('', {
                            line: comment[1].line,
                            ch: comment[1].ch - bcEnd.length
                        },
                        comment[1]);
                    doc.replaceRange('',
                        comment[0], {
                            line: comment[0].line,
                            ch: comment[0].ch + bcStart.length
                        });
                } else {
    ...
    function cursorAtAutoHint(cm, modeName, cursor, rightToken) {
        ...
        if (token.type === null && token.string === '.') {
            if (!rightToken) {
                return cursorAtAutoHint(cm, modeName, {
                    line: cursor.line,
                    ch: cursor.ch - 1
                }, token);
            }
    ...
    var modeMap = {
        ...
        'html': [
            ['vbscript', 'javascript', 'css', 'htmlmixed'], 'text/html'
        ],
        'css': [
            ['css'], 'text/css'
        ],
        ...
        if (codeFolding) {
            ...
            loadCSSList([require.toUrl('./css/codefolding.css')], function() {
                require(['webida-lib/custom-lib/codemirror/addon/fold/foldcode',
                    ...
                    'webida-lib/custom-lib/codemirror/addon/fold/comment-fold'
                ], function() {
                    self.addDeferredAction(function(self) {
